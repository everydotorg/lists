import { LocalCampaignInfo } from '../../../types/CampaignInfo'
export const demo: LocalCampaignInfo = {
  slug: 'demo',
  name: 'Demo List',
  primaryColor: '#00A37F',
  imageUrl: 'profile_pics/fzzktcwasfcaj9wxhdxd',
  mobileBannerUrl: 'profile_pics/fzzktcwasfcaj9wxhdxd',
  bannerUrl: 'profile_pics/fzzktcwasfcaj9wxhdxd',
  socialShareText: 'Help demo givelist.',
  previewImage: 'profile_pics/fzzktcwasfcaj9wxhdxd',
  about: 'Help demo givelist.',
  cause: 'ANIMALS',
  fundingGoal: 50000,
  nonprofits: [
    {
      slug: 'thehumaneleague',
      name: 'The Human League',
      location: 'Rockville, MD',
      about:
        'International nonprofit ending the abuse of animals raised for food.',
      img: 'faja_profile/s6cwebbnv283e86qwkfh'
    },
    {
      slug: 'globalfoodpartners',
      name: 'Global Food Partners Inc',
      location: 'Washington, DC',
      about:
        'We work with businesses to implement and maintain high animal welfare and responsible sourcing policies.',
      img: 'profile_pics/d3sdcot78tb0h5ovzgwu'
    },
    {
      slug: 'albertschweitzerfoundation',
      name: 'Albert Schweitzer Foundation',
      location: 'Berlin, Germany',
      about:
        'The Albert Schweitzer Foundation works to eliminate cruel farming practices in Europe.',
      img: 'profile_pics/zjee7iomkpax9bn0xa15'
    },
    {
      slug: 'gfi',
      name: 'The Good Food Institute',
      location: '',
      about:
        'We work with scientists, investors, and entrepreneurs to make groundbreaking good food a reality. We focus on clean meat and plant-based alternatives to animal products—foods that are more delicious, safer to eat, and better for the planet than their outdated counterparts.',
      img: 'faja_profile/j7pfmjuvafvflihgbaf3'
    },
    {
      slug: 'wildanimalinitiative',
      name: 'Wild Animal Initiative',
      location: 'Farmington, MN',
      about: 'We want to make life better for wild animals.',
      img: 'profile_pics/wzgsabl4bvjvfl6jezb1'
    }
  ]
}
